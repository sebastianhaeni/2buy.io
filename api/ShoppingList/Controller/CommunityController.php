...
class CommunityController extends BaseController
{
    ...
    public function create(Request $request, Application $app)
    {
        ...
        $communityHasUser = new CommunityHasUser($community->getId(), $app['auth']->getUser($request)->getId(), true, null, null, true);
    ...
    /**
     * Updates a the own preferences of a community.
     ...
     * @return \ShoppingList\Controller\Response
     */
    public function editOwnPreferences(Request $request, Application $app)
    {
        $checker = new CommunityChecker($request, $app, false);
        if (! $checker->isGood()) {
            return $checker->getResponse();
        }
        ...
        $communityHasUser = CommunityHasUser::getByUserId($user->getId(), $app);
        
        if ($user == null || $communityHasUser == null || count($communityHasUser) <= 0) {
            return new Response('Error', StatusCodes::HTTP_BAD_REQUEST);
        }
        ...
        $communityHasUser->setBankAccountNumber($request->get('bankAccountNumber'));
        $communityHasUser->setBankAccountName($request->get('bankAccountName'));
     ...
     * @return \Symfony\Component\HttpFoundation\JsonResponse
     */
    public function getOwnPreferences(Request $request, Application $app)
    {
        $checker = new CommunityChecker($request, $app, false);
        if (! $checker->isGood()) {
            return $checker->getResponse();
        ...
        return $this->json($user->getCurrentCommunitiyHasUser($checker->getCommunity()
            ->getId(), $app));
    }
}
